...
function resolveConfig(overrides?: Partial<CliConfig>): CliConfig {
  return {
...
function createGatewayClient(cfg: CliConfig, admin = false): AxiosInstance {
...
async function printJson(label: string, data: unknown) {
...
async function resolvePayload(inline?: string, filePath?: string): Promise<string> {
